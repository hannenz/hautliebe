...
<div id="cmtMessageContainer">
	{VAR:userMessage}
	{IF ({ISSET:cmtErrorMessage:VAR})}<div class="cmtMessage cmtMessageError">{VAR:cmtErrorMessage}</div>{ENDIF}
	{IF ({ISSET:cmtSuccessMessage:VAR})}<div class="cmtMessage cmtMessageSuccess">{VAR:cmtSuccessMessage}</div>{ENDIF}
</div>
{ENDIF}
...
{ENDIF}
{IF ({ISSET:newEntryLink:VAR} || {ISSET:showIpp:VAR} || {ISSET:showSelectedRowsInfo:VAR})}
...
{ENDIF}
{IF ({ISSET:currentRows:VAR} || {ISSET:showSelectedRowsInfo:VAR})}
